module VMC_print
    ...
    !####################################################       
    subroutine print_E_evolution_toFile(MC_step,E_avg,E,Epot,Ekin,Ekinfor)
        use VMC_parameters
        implicit none
        integer,intent(in) :: MC_step
        real*8, intent(in) :: E,Epot,Ekin,Ekinfor,E_avg
        ...
        if (FirstTime) then 
            ...
            open(FID,file=filename,status="replace")
            write(FID,"(A)") "step,Eavg,E,E^2,epot,ekin,ekinfor"
            FirstTime = .FALSE. 
            close(FID)
        end if

        open(FID,file=filename,access="append") 
        write(FID,*) MC_step,",",E_avg,",",E,",",E**2,",",Epot,",",&
                     Ekin,",",Ekinfor
        close(FID)
    end subroutine
